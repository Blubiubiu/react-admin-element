  ...
  render() {
    ...
    return (
      <div className="portrait">
        <div style={{backgroundImage:`url(${defaultStudentUrl})`}} className="portrait__wrapper">
          {this.state.headImg && (<img src={ this.state.headImg } alt="头像" />)}
        </div>
        <Button className="head" size="small">
          头像修改
          <input type="file" value={ this.state.file }  onChange={this.onSelectFile}></input>
        </Button>
        ...
        >
          <Dialog.Body>
            {src && ( <ImageCrop src={src} readCropImgUrl={this.readCropImgUrl} /> )}
          </Dialog.Body>

          <Dialog.Footer className="dialog-footer">
            <Button onClick={ () => this.setState({ cropDialog: false }) }>取 消</Button>
            <Button type="primary" onClick={ this.confirmHandle }>确 定</Button>
          </Dialog.Footer>
        </Dialog>
      </div>
    );
